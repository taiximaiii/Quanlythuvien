...
public class QuanLySach extends javax.swing.JFrame {
    ...
    public QuanLySach() {
        ...
        getNXB();
        getTheLoai();
        DataFromMySQL.getAndShowData(jTable_DSSach, columnTitlesOfJTable_DSSach, "SELECT S.*, TG.TENTACGIA, NXB.TENNXB, TL.TENTHELOAI FROM SACH S, TACGIA TG, NHAXUATBAN NXB, THELOAI TL WHERE S.MATACGIA = TG.MATACGIA AND S.MATHELOAI = TL.MATHELOAI AND S.MANXB = NXB.MANXB Order by TenSach");
    }
    ...
    private void initComponents() {
        ...
        if (jTable_DSSach.getColumnModel().getColumnCount() > 0) {
            jTable_DSSach.getColumnModel().getColumn(2).setHeaderValue("Ngày nhập");
            jTable_DSSach.getColumnModel().getColumn(4).setHeaderValue("Vị trí");
        }
    ...
    private void themMoiSach(String maSach, String tenSach, String maTacGia, String maNXB, String maTheLoai, String gia, String ngayNhap, String viTri, String soLuong) {
        String sql = "INSERT INTO SACH(MaSach,TenSach,NgayNhap,Gia,ViTri,MaTacGia,MaNXB,MaTheLoai,SoLuongCo,SoLuongCon) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        Connection con = KetNoiSQL.layKetNoi();
        try {
            PreparedStatement ps = con.prepareStatement(sql);
    ...
    private int kiemTraSachMuon(String maSach) {
        Connection con = KetNoiSQL.layKetNoi();
        int tonTai = 0;
        String sql = "SELECT * FROM PHIEUMUON WHERE MASACH = '" + maSach + "'";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    private void jButton_Them1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_Them1ActionPerformed
        ...
        if (maSach.equals("")) {
            ...
            if (kiemTraSach(maSach) == 1) {
                ...
                DataFromMySQL.getAndShowData(jTable_DSSach, columnTitlesOfJTable_DSSach, "SELECT S.*, TG.TENTACGIA, NXB.TENNXB, TL.TENTHELOAI FROM SACH S, TACGIA TG, NHAXUATBAN NXB, THELOAI TL WHERE S.MATACGIA = TG.MATACGIA AND S.MATHELOAI = TL.MATHELOAI AND S.MANXB = NXB.MANXB");
            }
        }
